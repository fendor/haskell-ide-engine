  ...
  pluginGetFile "getCompletions: " uri $ \file -> do
    ...
    ifCachedModuleAndData file (IdeResultOk [])
      $ \tm CachedInfo { newPosToOld } CC { allModNamesAsNS, unqualCompls, qualCompls, importableModules } ->
          let
            ctx = getContext pos (tm_parsed_module tm)
            ...
            -- | complete a module
            filtModNameCompls :: [J.CompletionItem]
            filtModNameCompls =
              map mkModCompl
                $ mapMaybe (T.stripPrefix enteredQual)
                $ Fuzzy.simpleFilter fullPrefix allModNamesAsNS

            -- | All completions that are in scope and belong to a certain module.
            -- If the module is empty, all completions are returned
            -- Module may also be the abbreviation, such as from an import
            --
            -- > import qualified Data.List as L
            --
            -- then "L" is a valid module name.
            filtCompls :: T.Text -> [CompItem]
            filtCompls prefixMod = Fuzzy.filterBy label prefixText ctxCompls
              where
                isTypeCompl = isTcOcc . occName . origName
                -- completions specific to the current context
                -- Default to value context if none given
                ctxCompls' = case fromMaybe ValueContext ctx of
                ...
                compls = if T.null prefixMod
                  then unqualCompls
                  else Map.findWithDefault [] prefixMod $ getQualCompls qualCompls

            -- | Import Completion. Complete an Import
            mkImportCompl :: T.Text -> J.CompletionItem
            mkImportCompl label = (J.detail ?~ label) . mkModCompl $ fromMaybe
              ""
              (T.stripPrefix enteredQual label)

            -- | Filter a list of texts with a fuzzy filter.
            filtListWith :: (T.Text -> a) -> [T.Text] -> [a]
            filtListWith f list =
              [ f label
            ...
            filtListWithSnippet :: (T.Text -> T.Text -> J.CompletionItem) -> [(T.Text, T.Text)] -> T.Text -> [J.CompletionItem]
            ...
            generalCompletions =
                filtModNameCompls
                ++ completionsForModule prefixModule

            completionsForModule m = map (toggleSnippets . mkCompl . stripAutoGenerated) (filtCompls m)
            ...
            result
              ...
              = generalCompletions
          in
            return . IdeResultOk $ case ctx of
              -- Just ValueContext -> generalCompletions
              -- Just TypeContext -> generalCompletions
              -- Just (ModuleContext _) -> [] -- TODO: offer completion with file name
              -- Just ExportContext -> []
              -- Just (ImportContext _) -> filtImportCompls
              -- Just (ImportListContext impName) -> generalCompletions
              -- Just (ImportHidingContext impName) -> completionsForModule (T.pack impName)
              -- Just ClassContext -> []
              -- Just InstanceContext -> []
              -- Just DerivingContext -> []
              _ -> result -- If we can not provide a context, but we still know what to complete
 where
